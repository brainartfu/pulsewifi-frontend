        ...
        <div class="navbar-header">
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="dropdown ms-sm-3 header-item topbar-user">
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                        <!-- item-->
                        <h6 class="dropdown-header welcome-greeting">Welcome</h6>
                        ...
                        <a class="dropdown-item " href="javascript:void();" onclick="logoutCurrentUser(event)"><i class="bx bx-power-off font-size-16 align-middle me-1"></i> <span key="t-logout">@lang('translation.logout')</span></a>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $('#page-header-user-dropdown .header-profile-user').attr('src', api.domain + 'storage/profile_img/avatar.png');
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if (!localStorage.getItem("token") || localStorage.getItem("token") == "") {
            ...
            if (params.has("token") && params.has("token_expire")) {
                for (const param of params) {
                    arr_param.push(param);
                }

                localStorage.setItem("token", arr_param[0][1]);
                localStorage.setItem("token_expire", arr_param[1][1]);

                $.ajax({
                    type: "get",
                    url: `${api.url}auth/profile`,
                    headers: {
                        Authorization: arr_param[0][1]
                    },
                    success: function(response) {
                        ...
                        $.ajax({
                            type: "get",
                            url: `${api.url}pdoa/get_pdoa/${window.location.host}`,
                            success: function(response) {
                                localStorage.setItem("pdoaInfo", JSON.stringify(response.data));
                            },
                            error: function(error) {
                                error.status == 404 && (window.location.href = "/404");
                            },
                        });

                        $.ajax({
                            type: "get",
                            url: `${api.url}cart/get_cart_amount/${currentUser.id}`,
                            headers: {
                                Authorization: localStorage.getItem("token")
                            },
                            success: function(response) {
                                if (response.data.model_count > 0) {
                                    $("#notification_cart").removeClass("hidden");
                                    $("#notification_cart").html(response.data.model_count);
                                }
                            },
                            error: (e) => {
                                ClearSession();
            ...
            currentUser.role < 4 && $("#btn_cart").addClass("hidden");

            let avatarStr = "";

            if (currentUser.profile_img_path != null) {
                let path = api.url + currentUser.profile_img_path;
                path = path.replace("api/public", "storage");
                avatarStr = `<img class="rounded-circle header-profile-user" src="${path}" alt="Header Avatar"></img>`;
            } else {
                avatarStr = `<div class="rounded-circle header-profile-user profile-no-avatar-nav">${
        currentUser.firstname.slice(0, 1).toUpperCase() + " " + currentUser.lastname.slice(0, 1).toUpperCase()
      }</div>`;
            }
      ...
      <div class="dropdown-menu dropdown-menu-end pt-0">
          <a class="dropdown-item" href="/profile"><i class='bx bx-user-circle text-muted font-size-18 align-middle me-1'></i> <span class="align-middle">My Profile</span></a>
          <a class="dropdown-item" id="logout" href="#"><i class='bx bx-log-out text-muted font-size-18 align-middle me-1'></i> <span class="align-middle">Logout</span></a>
      </div>`
            );
        }
    ...
    const userProfileImage = () => {
        let path = '';
        if (currentUser.profile_img_path) {
            path = currentUser.profile_img_path.replace("public", "storage");
        } else {
            path = 'storage/profile_img/avatar.png';
        }
        $('#page-header-user-dropdown .header-profile-user').attr('src', api.domain + path);
        let name = currentUser.firstname + " " + currentUser.lastname;
        $('#page-header-user-dropdown .user-name-text').html(name);
        $('.header-item.topbar-user .welcome-greeting').html(`Welcome ${name} !`);
    ...
    const userProfileRole = () => {
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    let role = response.data?.display_name;
                    $('#page-header-user-dropdown .user-name-sub-text').html(role);
                }
            },
        });
    };
    ...
    const ClearSession = () => {
        ...
        localStorage.setItem("token_expire", 0);
        localStorage.setItem("currentUser", JSON.stringify({}));
        localStorage.setItem("pdoaInfo", JSON.stringify({}));
