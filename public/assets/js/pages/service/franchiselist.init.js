...
$("#pdoa_id").on("change", (e) => {
    ...
    Pdoas.map((ele) => {
        if (ele.id == pdoaID) {
            ...
            setDistributorOptionList(pdoaID);
        }
    });
});

function setDistributorOptionList(pdoaID) {
    $.ajax({
        type: "get",
        url: api.url + "users/getPdoaDistributors/" + pdoaID,
        headers: {
            Authorization: localStorage.getItem("token"),
        },
        success: function (response) {
            $("#belongs_to").html("");
            let str = `<option value="">Select Distributor</option>`;
            response.data.map((ele) => {
                str += `<option value="${ele.id}">${ele.firstname} ${ele.lastname} [${ele.pdoa_id}]</option>`;
            });
            $("#belongs_to").html(str);
        },
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.data.Franchise) {
                $.ajax({
                    type: "get",
                    url: `${api.url}users/getAllFranchises/0/0`,
                    headers: {
                        Authorization: localStorage.getItem("token"),
                    },
                    success: function (response) {
                        Franchises = response.data;
                        setFranchisesListHtml(response.data);
                    },
                    error: function (error) {
                        error.status == 0 && (window.location.href = "/login");
                        error.status == 404 &&
...
function setPdoaOptionHtml(pdoaId = 0){
    $.ajax({
        type: "get",
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //                     setFranchiseListHtml(response.data);
    //                 },
    //                 error: function (error) {
    //                     error.status == 0 && (window.location.href = "/login");
    //                     error.status == 404 &&
...
const setFranchisesListHtml = (data) => {
    $html = "";
    if (data.length) {
        $.each(data, (index, row) => {
            $html += `<tr>
                    ...
                    <td class="id">${row.distributor.firstname} ${row.distributor.lastname}</td>
                    <td class="id">${row.total_locations}</td>
            ...
            $html +=
                ...
                `" ><i class="ri-eye-fill align-bottom me-2 text-muted"></i>
                                        View</button></li>
                                <li><button class="dropdown-item" href="javascript:void(0);" onclick="EditFranchise(this);" data-id="` +
                row.id +
                `" data-bs-toggle="modal"
                                        data-bs-target="#editFranchiseModal"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i>
                                        Edit</button></li>
...
function EditFranchise(data) {
    let franchise_no = data.getAttribute("data-id");
    localStorage.setItem("franchise-list", JSON.stringify(Franchises));
    localStorage.setItem("franchise_no", franchise_no);
    let filterFranchises = Franchises.filter(
        (franchise) => franchise.id == franchise_no
    );
    let franchise = {};
    let idProofs = ["Aadhaar", "Voter", "PAN"];
    if (filterFranchises.length > 0) {
        franchise = filterFranchises[0];
        $("#firstName").val(franchise.firstname);
        $("#lastName").val(franchise.lastname);
        $("#emailID").val(franchise.email);
        $("#phoneNumber").val(franchise.phone_no);
        $("#company_name").val(franchise.company_name);
        $("#designation").val(franchise.designation);
        $("#pdoa_id").val(franchise.pdoa_id);
        
        let distributorOptionHtml = `<option value="">Select Belongs to [Distributor]</option>`;
        franchise.distributors.forEach(function (distributor) {
            if (franchise.belongs_to == distributor.id) {
                distributorOptionHtml += `<option value="${distributor.pdoa_id}" selected="selected">${distributor.firstname} ${distributor.lastname}</option>`;
            } else {
                distributorOptionHtml += `<option value="${distributor.pdoa_id}">${distributor.firstname} ${distributor.lastname}</option>`;
            }
        });
        $("#belongs_to").html(distributorOptionHtml);

        let idProofHtml = `<option value="">Select ID Proof</option>`;
        idProofs.forEach(function (idProof) {
            if (idProof == franchise.id_proof) {
        ...
        $("#id_proof_no").val(franchise.id_proof_no);
        $("#address").val(franchise.address);
        $("#city").val(franchise.city);
        $("#state").val(franchise.state);
        $("#country").val(franchise.country);
        $("#postal_code").val(franchise.postal_code);
        $("#gst_no").val(franchise.gst_no);
        $("#revenue_model").val(franchise.revenue_model);
        $("#revenue_sharing_ratio").val(franchise.revenue_sharing_ratio);
        $("#beneficiary_name").val(franchise.beneficiary_name);
        $("#ifsc_code").val(franchise.ifsc_code);
        $("#ac_no").val(franchise.ac_no);

        let backend_path = api.domain;

        $("#preview_passbook_cheque").html("");
        let arr_exe_ = franchise.passbook_cheque.split(".");
        if (
            ...
            $("#preview_passbook_cheque").append(
                `<img src="${
                    backend_path +
                    franchise.passbook_cheque.replaceAll("public", "storage")
