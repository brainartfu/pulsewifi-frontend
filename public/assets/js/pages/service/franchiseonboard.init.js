...
function getInventoryAndUserFees() {
    $.ajax({
        ...
        success: function (response) {
            if (response.data.Franchise && currentUser.role < 4) {
                ...
                $.ajax({
                    type: "get",
                    url: `${api.url}users/getPdoaDistributors/${
                        JSON.parse(localStorage.getItem("pdoaInfo")).id
                    }`,
                    headers: { Authorization: localStorage.getItem("token") },
                    success: function (response) {
                ...
                $.ajax({
                    ...
                    headers: { Authorization: localStorage.getItem("token") },
                    success: function (response) {
                        franchiseFee = response.data.franchise_fee;
                    },
                    error: function (error) {
                        error.status == 401 && ClearSession();
                    },
                ...
                if (ele.id == inventoryID) {
                    ...
                    if (invoiceList.length == 0) {
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.success) {
                                    ...
                                    $("#oq-4 .error_message").html(response.message);
                        ...
                        if (addFlag) {
                            $.ajax({
                                type: "get",
                                url: `${api.url}cart/add_cart/${distributorID}/${ele.id}/${Number(quantity)}`,
                                headers: {
                                    Authorization:
                                        localStorage.getItem("token"),
                                },
                                success: function (response) {
                                    if (response.success) {
                                        data.push(response.data[response.data.length - 1].id);
                                        invoiceList.push(data);
                                        ele.left_inventory -= Number(quantity);
                                        $("#max_quantity").val(ele.left_inventory);
                                        makeInvoiceList();
...
function submitFranchiseDetail() {
    let formData = new FormData();

    $("#upload_id_proof").prop("files")[0] &&
        formData.append(
            "upload_id_proof",
            $("#upload_id_proof").prop("files")[0]
        );
    ...
    formData.append("identity_verification", $("#identity_verification").val());
    formData.append("belongs_to", $("#belongs_to").val());
    ...
    if (franchiseID == -1) {
        $.ajax({
            type: "post",
            url: `${api.url}users/add_franchise/${
                JSON.parse(localStorage.getItem("pdoaInfo")).id
            }`,
            headers: { Authorization: localStorage.getItem("token") },
            data: formData,
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    franchiseID = response.data.id;

                    $.ajax({
                        type: "get",
                        url: `${api.url}cart/add_cart/${franchiseID}/franchise_fee/${franchiseFee}`,
                        headers: {
                            Authorization: localStorage.getItem("token"),
                        },
                        success: function (response) {
        ...
        $.ajax({
            type: "post",
            url: `${api.url}users/updateuser/${franchiseID}`,
            headers: { Authorization: localStorage.getItem("token") },
            data: formData,
            contentType: false,
            processData: false,
...
function submitOrderCreation() {
    $.ajax({
        ...
        success: function (response) {
            ...
            setTimeout(function () {
                $("#oq-4 .success_message").fadeOut("slow");
                window.location.href = "/service/franchise/manage-franchise";
            }, 1000);
        },
        error: function (error) {
            error.status == 401 && ClearSession();
...
function makeInvoiceList() {
    ...
    str = `<table class="table align-middle table-nowrap table-centered mb-0">
              ...
              <tbody>
                  <tr>
                      <td>
                          <h5 class="text-truncate font-size-14">Franchise Fee</h5>
                      </td>
                      <td>₹ ${franchiseFee}</td>
                      <td>1</td>
                      <td class="text-end">₹ ${franchiseFee}</td>
                      <td></td>
                  </tr>
                  ${str_}
                  <tr>
