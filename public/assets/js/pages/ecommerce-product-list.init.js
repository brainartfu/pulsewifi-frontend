...
if (inputValueJson) {
  ...
  inputValueJson.forEach(function (element) {
...
var productListAll = new gridjs.Grid({
  columns: [{
    ...
    formatter: function formatter(cell, row) {
      ...
      return gridjs.html('<div class="dropdown">' + '<button class="btn btn-soft-secondary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">' + '<i class="ri-more-fill"></i>' + '</button>' + '<ul class="dropdown-menu dropdown-menu-end">' + '<li><a class="dropdown-item" href="apps-ecommerce-product-details.html"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>' + '<li><a class="dropdown-item edit-list" data-edit-id=' + x + ' href="apps-ecommerce-add-product.html"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>' + '<li class="dropdown-divider"></li>' + '<li><a class="dropdown-item remove-list" href="#" data-id=' + x + ' data-bs-toggle="modal" data-bs-target="#removeItemModal"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>' + '</ul>' + '</div>');
    }
  }],
  className: {
    th: 'text-muted'
...
var productListPublished = new gridjs.Grid({
  columns: [{
    ...
    formatter: function formatter(cell, row) {
      return gridjs.html('<div class="dropdown">' + '<button class="btn btn-soft-secondary btn-sm dropdown" type="button" data-bs-toggle="dropdown" aria-expanded="false">' + '<i class="ri-more-fill"></i>' + '</button>' + '<ul class="dropdown-menu dropdown-menu-end">' + '<li><a class="dropdown-item" href="apps-ecommerce-product-details.html"><i class="ri-eye-fill align-bottom me-2 text-muted"></i> View</a></li>' + '<li><a class="dropdown-item" href="apps-ecommerce-add-product.html"><i class="ri-pencil-fill align-bottom me-2 text-muted"></i> Edit</a></li>' + '<li class="dropdown-divider"></li>' + '<li><a class="dropdown-item remove-list" href="#" data-id=' + row._cells[0].data + ' data-bs-toggle="modal" data-bs-target="#removeItemModal"><i class="ri-delete-bin-fill align-bottom me-2 text-muted"></i> Delete</a></li>' + '</ul>' + '</div>');
    }
  }],
  className: {
    th: 'text-muted'
...
}).render(document.getElementById("table-product-list-published")); // Search product list
...
document.querySelectorAll('.filter-list a').forEach(function (filteritem) {
...
var filterDataAll = '';
var filterDataPublished = ''; // When the slider value changes, update the input and span
...
document.querySelectorAll(".filter-accordion .accordion-item").forEach(function (item) {
  ...
  item.querySelectorAll(".form-check .form-check-input").forEach(function (subitem) {
...
function checkRemoveItem() {
  ...
  setTimeout(function () {
    document.querySelectorAll(".checkbox-product-list input").forEach(function (item) {
...
function removeItems() {
  ...
  removeItem.addEventListener('show.bs.modal', function (event) {
    ...
    document.getElementById("delete-product").addEventListener("click", function () {
      document.querySelectorAll(".gridjs-table tr").forEach(function (element) {
        var filtered = '';
...
function removeSingleItem() {
  var getid = 0;
  document.querySelectorAll(".remove-list").forEach(function (item) {
    item.addEventListener('click', function (event) {
  ...
  var getEditid = 0;
  document.querySelectorAll(".edit-list").forEach(function (elem) {
